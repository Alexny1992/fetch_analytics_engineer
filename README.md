...
He or she scans a receipt through the app and create essential information to the receipt table - a user can have multiple receipts (1 to many relationship)
However, a receipt can contain Multiple items and multiple receipts can also be from the same item therefore this is a many-to-many relationship  
Within the receipt item table, Barcode column connects to Brands table. Multiple items can be under one brand (Many to one relationship)
Finally, a brand can only be under one category so this will be a one to one relationship 
